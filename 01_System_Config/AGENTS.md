...
### 🔪 大文件防御机制 (doc-slicer) [2026-03-01]
- 这是应对超大 PDF 和数据包的最高防爆指南。
- 一旦接手大文件，禁止直接回答！必须先提出**分段切割、隔离压缩**的预估时间并得到 LC 许可。
- 读一段 -> 抽干水分保存至本地 Markdown -> 烧掉废话缓存 -> 再读下一段。严禁在大文件对话流内滚雪球。

### 🧬 凌晨自进化与捕猎指令 (2026-03-01)
- 每天凌晨 4 点，派遣一只精锐子智能体去外网 (X, GitHub, Reddit) 进行游猎。
- 不仅要带回**大盘新闻与 Agent 圈前沿讨论**，更被要求从开源池 (ClawHub 等) 里挑选至少 1 个适合自身的技能（特别是关于系统稳定性或自动化扩张的技能）去解构学习，并将原理吞纳入 `EVOLUTION.md`。
- 这些凌晨的进化果实，最终成为 LC 第二天桌边早安快报的一部分。

### 💰 终极 Token 守财奴防线设计 (2026-03-01 晚间)
- **模型隔离法**：所有大体积脏活（读PDF、粗排长文）必须派发给 `worker (MiniMax-M2.5)` 节点，因为国内 API 跑这些粗活极其廉价！
- **禁止乱用大模型**：主节点 `main (Gemini 3.1 Pro)` 的2M上下文专供 LC 陪聊和最终分析用，不接手任何数百MB未压缩的原始数据！
- **强制 Web 限流**：系统内置网页抓取 () 硬限制在单次被掐至 1.5 万字以下，杜绝爬虫行为引发的 Token 爆仓泄漏。

### 💰 终极 Token 守财奴防线设计 (2026-03-01 晚间)
- **模型隔离法**：所有大体积脏活（读PDF、粗排长文）必须利用子进程或工具派发给 worker 节点(如采用国内廉价但极强的 MiniMax-M2.5)，将海量输入丢给最便宜的点。
- **禁止乱用主模型**：主节点 main (Gemini 3.1 Pro)专供高级逻辑，禁止直接加载大体量的原始报表文件以防吃单价。
- **强截断限制**：为了拦截爬网页浪费 Token，系统已经在底层强制锁定请求网页时单次不能超过 1.5 万字截断。
